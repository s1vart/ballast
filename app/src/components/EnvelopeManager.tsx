...
import { colors, money, paletteFor } from '../theme';
    ...
    <BottomSheet visible={visible} onClose={close} title={editing ? (editing === 'new' ? 'New envelope' : 'Edit envelope') : 'Envelopes'}>
      {editing ? (
        <EnvelopeForm
          ...
          onSave={async (f) => {
            if (editing === 'new') { await addCategory(f.name, f.monthlyLimit); toast('Envelope added'); }
...
function EnvelopeForm({
  ...
  cat: Category | null;
  onSave: (f: { name: string; monthlyLimit: number }) => void;
  onDelete?: () => void;
  onCancel: () => void;
}) {
  ...
  return (
    <View>
      ...
      <PrimaryButton
        ...
        disabled={!valid}
        onPress={() => valid && onSave({ name: name.trim(), monthlyLimit: parsed })}
      />
